...
contract.startListenRes = function () {
    this.contract.events.resEndEvent()
        .on('data', async (result) => {
            try {
                ...
                if (files.length !== 0) {
                    ...
                    let times = Math.ceil(files.length / 10);
                    for (var batchnum = 0; batchnum < times; batchnum++) {
                        let batchFiles = files.slice(batchnum * 10, (batchnum + 1) * 10)
                        let text = ''
                        const sendTime = Date.now()
                        batchFiles.map(file => text += JSON.stringify(file.shareWarrant))
                        text += reqID + sendTime
                        const unionSign = crypto.eccSign(crypto.hash(text))
                        ...
                        const response = await axios({
                            ...
                            data: { reqID, batchnum, batchFiles, sendTime, unionSign }
                        })
                    ...
                    const response = axios.post(`${bank.url}/response-file`, { reqID })
                    Logger.log(response.data)
                }
            } catch (error) {
                Logger.error(error.stack)
